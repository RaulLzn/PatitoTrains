...
import patitotrains.controller.userManagementController.interfaces.RegisterUserControllerInterface;
...
public class RegisterUserController implements Initializable, RegisterUserControllerInterface {
    ...
    public void btnAddClicked(ActionEvent event) throws NotBoundException, RemoteException {
        ...
        if(valuesFormatOk){
            ...
            if(toAdd){
                User userToAdd = new User(name, lastName, nmrs,false, id, user, password );
                ...
                if(userManager.addUser(userToAdd)){
                    ...
                    log.logAction("Registro de usuario: " + name + " " + lastName  +" con id " + id + " registrado con el usuario " + user + ".");
                    clear();
                }else{
